...
>[!IMPORTANT] This would not have been possible to write had it not been for the hardwork knutwalker put in writing the autosplitter for FF1 Pixel Remaster. I've merely used the framework he used for FF1 (with permission!) and applied it to FF2.
> Knutwalker deserves the credit, not me!

An auto splitter for Final Fantasy 2 Pixel Remaster.
...
Requires manual split on final boss (sorry!)
...
- In the splits editor, make sure that the game is 'Final Fantasy II' so that LiveSplit can find the autosplitter.
...
Some people may prefer to split on key items, others on Boss Fights so I have tried to accomodate all.
...
> That is, your route might go to the Tropical Forest before Deist
...
The debugger is able to step through the code. You can set breakpoints in VSCode and it should stop there when the breakpoint is hit.
Inspecting variables may not work all the time.
